...
mkdir -p cmake/build
cd cmake/build
# Do NOT put %%{buildroot} into gRPC_INSTALL_*DIR — it gets baked into
# gRPCTargets.cmake and fails RPM check-buildroot.
cmake ../../ -DgRPC_INSTALL=ON\
             ...
             -DgRPC_INSTALL_CMAKEDIR=%{_prefix}/lib/cmake/%{name} \
             -DgRPC_INSTALL_SHAREDIR=%{_datadir}/%{name} \
             -DgRPC_INSTALL_PKGCONFIGDIR=%{_libdir}/pkgconfig \
...
%install
cd cmake/build
# Use DESTDIR instead of install/local with buildroot-prefixed INSTALL_*DIR
...
%changelog
* Tue Aug 11 2026 local <ceph@localhost> - 1.60.0-5.1
- Ship CMake configs in grpc-devel (gRPCConfig.cmake) for Ceph
- Fix gRPC_INSTALL_*DIR/DESTDIR to avoid check-buildroot failure
